...
static config_var_t option_vars_[] = {
  ...
  V(ClientOnly,                  BOOL,     "0"),
  V(CIRC_NEEDED,                        INT,     "2"),//Mohsen
  V(MYLAT,                        INT,     "32"),//Mohsen
  V(MYLON,                        INT,     "97"),//Mohsen
  V(MINGAIN,                      INT,     "25"),//Mohsen
  V(BETA,                         INT,     "4"),//Mohsen
  ...
  V(CONG_TIME_DEFAULT,                 INT,     "100"),//Mohsen
  V(GeoPath,              STRING,   "/usr/local/lib/GeoLiteCity.dat"),//Mohsen
  V(MYIP,              STRING,   "127.0.0.1"),//Mohsen
  V(COMBINED_WEIGHT,                     INT,     "1"),//Mohsen 1 combined weighting, else breaking ties
  V(CIRC_SELECT,                     INT,     "6"),//Mohsen 1 means distance then congestion, 3 median rtts, else congestion then distance
  V(GAMMA,                 INT,     "0"),// GAMMA zero: we skip the new weight function; keep in mid that GAMMA = 100 is s = 1, and Also, BETA should be less than zero
  V(OPTIMAL,                        INT,     "0"),//Mohsen if it's one then it select the shortest path.
  V(VANILLA,                        INT,     "0"),//Mohsen VANILLA == 0 (our design) == 2 (CAR) .
  V(CAR,                        INT,     "0"),//Mohsen Congestion-aware.
  V(UPDATE_S,                        INT,     "1"),//Mohsen, it finds the best s close to the weight selection with bandwidth
  V(BUCKET,                        INT,     "40"),//
  V(CUTOFF,                        INT,     "600"),// circuits congested more that this will be ignored
  V(POWER_EXIT,                        INT,     "350"),// the power used in computing the weight in Wd 
  V(POWER_MIDDLE,                        INT,     "400"),// the power used in computing the weight in Wd
  V(POWER_GUARD,                        INT,     "350"),// circuits congested more that this will be ignored
  V(POP_DEST_NUM,                        INT,     "2"),//the number of popular destination in our design, they can be 2 or 4
  V(OPEN_CIRCUITS_ISEC,          INT,   "3"),
  V(OPEN_CIRCUITS_BOOTSTRAP_ISEC,          INT,   "3"),
  V(OPEN_CIRCUITS_PER_PORT_ISEC,          INT,   "2"),
  V(ISEC,          INT,   "1"),
  V(CIRC_NUM_RANDOM_ISEC,          INT,   "3"),
  V(CIRC_LIFE_TIME_ISEC,          INT,   "5"),
  V(MYLOG,          INT,   "0"),
//iSec----------------------------------------------
