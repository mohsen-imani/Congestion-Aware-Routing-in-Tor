...
if (get_options()->ClientOnly == 1){
	  if (begin_type == RELAY_COMMAND_BEGIN){
	  	...
	  	edge_conn->base_.flag_begin = 1;
	  	tor_gettimeofday(&edge_conn->base_.begin_time);// 
		char* source = getOurIP();
		tor_log(LOG_NOTICE, LD_CONTROL,"@iSec:%s:ASpath:Client-IP:%s:Entry-IP:%s:Entry-BW:%u:Middle-IP:%s:Middle-BW:%u:Exit-IP:%s:Exit-BW:%u:Destination-IP:%s:CIRC-RAND-NO:%d:Circ-RTT:%ld:Stream-ID:%d:CIRC-RAND-NO:%d:CIRC-ID:%u:Done",source,source, circ->base_.entry_ip, circ->base_.gbw,circ->base_.middle_ip,circ->base_.mbw,circ->base_.exit_ip, circ->base_.ebw,ap_conn->socks_request->address, circ->base_.rand_no, get_mean_list(TO_CIRCUIT(circ), 1),edge_conn->stream_id, circ->base_.rand_no, (unsigned)circ->base_.n_circ_id);
  }
}
//iSec--------------------------------------------------
