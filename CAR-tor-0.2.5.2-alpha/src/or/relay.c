...
//iSec:----------------------------------------------
  if (get_options()->ClientOnly == 1){
	  if (relay_command == RELAY_COMMAND_DATA){
		++circ->no_data;
		if (circ->no_data == CIRCWINDOW_INCREMENT){
		    time_sendmes *tt = tor_malloc(sizeof(time_sendmes));
		    tt->flag_sent = ++(circ->no_sent);
		    tor_gettimeofday(&tt->t);
		    smartlist_add(circ->times, tt);
		    circ->no_data = 0;
		    log_info(LD_APP,"@iSec:%s:WAITING FOR CIRC SENDME:CIRC MYID:%d:NO:%d:",get_options()->MYIP,circ->rand_no,tt->flag_sent);
...
//iSec:----------------------------------------------
  if (get_options()->ClientOnly == 1){
	  if (fromconn && relay_command == RELAY_COMMAND_DATA){
		++fromconn->base_.no_data;
		if (fromconn->base_.no_data == STREAMWINDOW_INCREMENT){
		    time_sendmes *tt = tor_malloc(sizeof(time_sendmes));
		    tt->flag_sent = (++(fromconn->base_.no_sent));
		    tor_gettimeofday(&tt->t);
		    smartlist_add(fromconn->base_.times, tt);
		    fromconn->base_.no_data = 0;
	      log_info(LD_APP,"@iSec:%s:WAITING FOR STREAM SENDME:STREAM:%d:NO:%d:",get_options()->MYIP,fromconn->stream_id,tt->flag_sent);
		}
	   }
   }
...
//iSec-----------------------------------------
   if (get_options()->ClientOnly == 1){
	    if (conn->base_.flag_begin == 1){
	       struct timeval t2;
	       tor_gettimeofday(&t2);
	       long rtt = tv_mdiff(&conn->base_.begin_time, &t2);
	       add_to_list(circ, rtt);
	       conn->base_.flag_begin = 0;
	     }//if
   }
//----------------------------------------------------
...
//iSec: ------------------------------------------------------------
        if (get_options()->ClientOnly == 1){
		(circ->no_extended)++;
		if (circ->no_extended == 2){
			log_info(LD_APP,"@iSec:%s:Second Extended received:Circ-ID:%u",get_options()->MYIP, (unsigned)circ->n_circ_id);
			tor_gettimeofday(&circ->end);
			long rtt = tv_mdiff(&circ->start, &circ->end);
			add_to_list(circ,rtt);
		}



		if (circ->no_extended == 1) {
		tor_gettimeofday(&circ->start);
...
//iSec:--------------------------------------------------
        if (get_options()->ClientOnly == 1){
		log_info(LD_APP,"@iSec:%s:CIRC SENDME RECEIVED",get_options()->MYIP);
		struct timeval t4;
		tor_gettimeofday(&t4);
		    int cnm1 = 0;
		    int tttt1 = 0;
		    int tmp4 = 0;
		    time_sendmes *tmp6 =NULL;
		    ++circ->no_received;
		    SMARTLIST_FOREACH_BEGIN(circ->times, time_sendmes *, tm) {
		        tttt1++;
	      		if (circ->no_received == tm->flag_sent){
			    long rtt = tv_mdiff(&tm->t, &t4);    
		 	    cnm1 = 1;
			    tmp4 = tttt1;
			    tmp6 = tm;
			    add_to_list(circ,rtt);
			}//inner if
		   }SMARTLIST_FOREACH_END(tm); 
		   if (cnm1 == 1){
		       smartlist_del(circ->times, tmp4-1);
		       tor_free(tmp6);
		   }
           }
...
//iSec:--------------------------------------------------
	if (get_options()->ClientOnly == 1){    
		log_info(LD_APP,"@iSec:%s:STREAM SENDME RECEIVED",get_options()->MYIP);    
		struct timeval t3;
		tor_gettimeofday(&t3);
		if (conn){
		    int cnm = 0;
		    int tttt = 0;
		    int tmp3 = 0;
		    time_sendmes *tmp2 =NULL;
		    ++conn->base_.no_received;
		    SMARTLIST_FOREACH_BEGIN(conn->base_.times, time_sendmes *, tm) {
		        tttt++;
	      		if (conn->base_.no_received == tm->flag_sent){
			    long rtt1 = tv_mdiff(&tm->t, &t3);    
		 	    cnm = 1;
			    tmp3 = tttt;
			    tmp2 = tm;
		            add_to_list(circ,rtt1);
			}//inner if
		   }SMARTLIST_FOREACH_END(tm); 
		   if (cnm == 1){
		       smartlist_del(conn->base_.times, tmp3-1);
		       tor_free(tmp2);
		   }
		}//outer if
	}
