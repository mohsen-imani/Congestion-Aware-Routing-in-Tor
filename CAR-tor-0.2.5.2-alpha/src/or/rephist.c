...
void add_to_list(circuit_t *circ, long rtt){
	int indx = (++circ->rear) % LIST_LEN_MOH;
        circ->rtts[indx] = rtt;
	if (rtt < circ->min){
		circ->min = rtt;
		circ->ind_min = indx;
	}
	log_info(LD_CIRC,"@iSec:%s:ADD RTT:CIRC:%u:MYID:%d:ADDED RTT:%ld:MIN RTT:%ld:MIN INDEX:%d:CURRENT INDEX:%d:DONE",get_options()->MYIP, circ->n_circ_id, circ->rand_no,rtt,circ->min, circ->ind_min, circ->rear);
        return;
}
...
long get_mean_list(circuit_t *circ, int a){
	if (a == 1){
		int len = get_length_of_list(circ);
		int rear = circ->rear;
		if (rear == -1 || len == 0){
			return get_options()->CONG_TIME_RTT;
		}
		long rtts_tmp[LIST_LEN_MOH];
		int t = 0;
		for (t = 0; t< len; t++){
			rtts_tmp[t] = circ->rtts[rear%LIST_LEN_MOH];
			rear--;	
    		...
    		for(i=0;i<len;i++){
        		for(j=i+1;j<len;j++) {
            			if(rtts_tmp[i]>rtts_tmp[j]) {
                			temp=rtts_tmp[j];
                			rtts_tmp[j]=rtts_tmp[i];
                			rtts_tmp[i]=temp;
            			}
        		}
		}
        	...
        	if(len%2==0) {
            		return (rtts_tmp[len/2]+rtts_tmp[len/2-1])/2;
        	}else{
            		return rtts_tmp[len/2];
        	}
		...
		if (i == 0 || len == 1){
			return get_options()->CONG_TIME_DEFAULT;// default
		}
		long tmp = 0;
		int cntr = 0;
		while(m >=1 && j >= 1){
			if (circ->rtts[i%LIST_LEN_MOH] == circ->min && circ->ind_min == i%LIST_LEN_MOH){
				i--;
				m--;
				j--;
			}else{
				tmp = tmp + circ->rtts[i%LIST_LEN_MOH];
				i--;
				m--;
				j--;
				cntr++;
...
void print_list(circuit_t *circ){
	...
	int m = len;
	int i = circ->rear;
	log_info(LD_APP,"@iSec:%s:list length: %d, rear: %d",get_options()->MYIP,m,i);
	if (i == -1 || len == 0){
           log_info(LD_APP,"@iSec:%s**:(print function) 500",get_options()->MYIP); 
	   return;
	}
	while(m >=1){
		log_info(LD_APP,"@iSec:%s*:(print function):CIRC ID:%u:RTT:%ld",get_options()->MYIP,circ->n_circ_id, circ->rtts[i%LIST_LEN_MOH]);
		i--;
		m--;
...
{
    const weight_idx *p1 = (weight_idx *)v1;
    const weight_idx *p2 = (weight_idx *)v2;
...
int* get_rand(int c){
	...
	int *rnd = (int*)malloc(c*sizeof(int));
        if (rnd == NULL){
		return NULL;
	 }
	for (i = 0; i < c; i++){
	...
	int tmp_rnd;
	int c_tmp = c;
        int *rnd_3 =  (int*)malloc(poll*sizeof(int));
        if (rnd_3 == NULL){
		return NULL;
	 }
	int j;
...
circuit_t * circuit_get_by_id(circid_t id) {
  circuit_t *circ;
  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head){   //  SMARTLIST_FOREACH_END(circ);
      if (circ->rand_no  == id){
         return circ;
      }
  }
