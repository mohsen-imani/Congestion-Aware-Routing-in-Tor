                      ...
                      time_t now)
{
  ...
  circuit_t *circ_ = TO_CIRCUIT(origin_circ);//iSec
  tor_assert(circ);
  tor_assert(conn);
...
//iSec:---------------------------------------------
      if ((get_mean_list(circ_, 5)> (get_options()->CUTOFF)) && ((get_options()->ClientOnly == 1) && !need_internal && purpose == CIRCUIT_PURPOSE_C_GENERAL && !(origin_circ->build_state->onehop_tunnel) && !conn->want_onehop)){
	log_info(LD_CIRC,"@iSec:%s:IGNORED:NOT ACCEPTED:DONE:%u:RTT:%d",get_options()->MYIP, (unsigned)circ_->n_circ_id,(int)get_mean_list(circ_, 5));
        return 0;
      }	
//iSec:---------------------------------------------
...
circuit_get_best_(const entry_connection_t *conn,
                 int must_be_open, uint8_t purpose,
                 int need_uptime, int need_internal)
{
  circuit_t *circ;
...
static origin_circuit_t *
circuit_get_best_CAR(const entry_connection_t *conn,
                 int must_be_open, uint8_t purpose,
                 int need_uptime, int need_internal)
{


  if (!((!need_internal && (purpose == CIRCUIT_PURPOSE_C_GENERAL) && !conn->want_onehop))){
	log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: Vanilla", get_options()->MYIP); 
	return circuit_get_best_(conn, must_be_open, purpose, need_uptime, need_internal);
  }
  ...
  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) {
   ...
   log_info(LD_REND|LD_CIRC,"@iSec:%s:CIRCUITS-AVAILABLE:%d:DONE", get_options()->MYIP,cntr);
  int poll = get_options()->CIRC_NUM_RANDOM_ISEC;
  //-------------------------------------------------------
  rnd = get_rand(cntr);// do not forget to free 'rnd'
  if (rnd == NULL || cntr == 0 ){
        free(rnd);
	return NULL;
  }
  int len = (cntr < poll)? cntr:poll;
  list = (int*)malloc(len*sizeof(int));//do not forget to free 'list'
  if (list == NULL){
        free(rnd);
  	free(list);
	return NULL;
  ...
  for (j = 0; j < len ; j++ ){
       log_info(LD_REND|LD_CIRC,"@iSec:%s: Random numbers %d", get_options()->MYIP, rnd[j]);
  }
  ...
  for (j = 0; j < len ; j++ ){
	circ = circuit_get_by_id(list[j]);
	tmp = get_mean_list(circ, 5);
	log_info(LD_REND|LD_CIRC,"@iSec:%s:TOP-CIRC:MYID:%d:entry:%s:middle:%s:exit:%s:CONG:%ld:DONE", get_options()->MYIP, circ->rand_no,circ->entry_ip, circ->middle_ip, circ->exit_ip,tmp);
	if (tmp < min){
		min = tmp;
	        best = TO_ORIGIN_CIRCUIT(circ);
	}
  }
...
if (best)
	log_info(LD_REND|LD_CIRC,"@iSec:%s:SELECTED-CIRC-MYID:%d:", get_options()->MYIP, best->base_.rand_no);
  free(rnd);
  free(list);
  return best;
}
                 ...
                 int need_uptime, int need_internal)
{
          ...
          if ((get_options()->ClientOnly == 0) && !((!need_internal && (purpose == CIRCUIT_PURPOSE_C_GENERAL) && !conn->want_onehop))){
		log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: Vanilla", get_options()->MYIP); 
		return circuit_get_best_(conn, must_be_open, purpose, need_uptime, need_internal);

	  }else{

		log_info(LD_REND|LD_CIRC,"@iSec:%s: Circ Selection: CAR", get_options()->MYIP);  
		return circuit_get_best_CAR(conn, must_be_open, purpose, need_uptime, need_internal);
	}
